...
-- What if we wanted all the capital cities?
...
SELECT * FROM cities WHERE province = 'ON' AND population > 100000;

-- and we can list out results any way we like. Here, we're listing them in descending order.
...
-- By adding a comma and a number after the limit, the system knows to start getting records from there.


/*
    This is a 'Canadian Cities' database, but we actually have cities, towns, and villages.
    They're each defined by their population sizes as follows:
...
-- So, how might we select only cites?
