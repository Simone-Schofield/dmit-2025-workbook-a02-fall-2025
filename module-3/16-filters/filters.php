 ...
 * $filter - the CATEGORY you're toggling (i.e. continent, wellbeing ...)
 * $value - the specific VALUE you're toggling (e.g. europe, 6-8 ...)
*/
function build_query_url($base_url, $filters, $filter, $value) {
    // This function starts by copying the existing the filters into a new variable. This ensures that the original $filters remain unchanged while we modify the copy.
    ...
    */
    if (isset($updated_filters[$filter]) && in_array($value, $updated_filters[$filter])) {
        ...
        if (empty($updated_filters[$filter])) {
            ...
            // If the filter does not already exist in our query string, we know it's a new value and we need to add it in.
...
// We start by seeing if the user chose any filters (i.e. if any filters are active).
$active_filters = [];
...
foreach ($filters as $filter => $options) {
    // We'll start by taking the category names and 'translating' them into headings for each catergory.
