<?php
/*
    These two lines of codde import our credentials (i.e., the things MySQLi needs to have in order to be authenticated and access the database) and create a connection handle. We use 'require' here than 'include' because if the file cannot be found or if PHP can't log into the database, the engine will throw a fatal error and the page won't load.
        ...
        <main class="row justify-content-center">
            <section class="col col-md-10 col-lg-8 col-xxl-6">
                ...
                <?php
                    ...
                    if (mysqli_num_rows($result) > 0) {
                        // Our biggest challenge here is... English. We need to present our results in a context or a way that makes sense to the user. In this case, we want to create a list items where there's a comma (,) after each item -- except for the very last one. For the last item, we need an 'and' before it.
                        ...
                        // Finally, we can output out results to the user. Here, we're using an array implosion method that takes all of the items in the $cities[] array that we just made and collaspes them into a single string.
                ...
                <?php
                    ...
                    if (mysqli_num_rows($result) > 0) {
                        ...
                        // So far, this is just like the last time we imploded an array into a list of city names. This time, let's make it a bit tricker: let's make the last item have an ", and" before it.
                        
                        // This grabs the last item in $cities, remove it from the array (pops it off) and assigns it to a string variable instead.
                        $last_city = array_pop($cities);
                        $city_list = implode(', ', $cities);

                        // There may be case where there's only one city. This control structure accounts for that scenario.
                        if (!empty($city_list)) {
