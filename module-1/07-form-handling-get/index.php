<?php
    ...
    // Based upon what number the user gave us, we can now set our multipler or coefficient for our indgredient quantities. WE'll use a filter here, which just double-checks to make usre that whatever number they gave us was an integer. This is an important step because of the user can muck around with any value in the query string.
    ...
    // The base recipe is for 12 muffins, so we'll figure out multiplier by dividing this by 12.
    $multiplier = $recipe_yield / 12;

    // Our oven temperature is either 325F or 165C, depending upon what the user selects. We'll use Celsius as our default.
    $temperture_units = isset($_GET['temperature']) ? htmlspecialchars($_GET['temperature']) : 'C';
    $oven_temperature = ($temperture_units == 'C') ? '165&deg;C' : '325&deg;F' ;
        ...
        <form action="<?= htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="GET" class="p-5">
            ...
            <!-- Temperature -->
             <fieldset class="mb-3">
                ...
                <input type="radio" name="temperature" id="temperature-c" value="C" class="btn-check" <?php if ($temperture_units == 'C') echo 'checked'; ?>>
                <label for="temperature-c" class="btn btn-sm btn-outline-primary">Celsius</label>

                <input type="radio" name="temperature" id="temperature-f" value="F" class="btn-check" <?php if ($temperture_units == 'F') echo 'checked'; ?>>
