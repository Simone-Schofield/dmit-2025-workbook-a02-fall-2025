...
// This is the start of our test 'gauntlet', where we perform mulitple tests on each piece of data that the user gave us.
...
if (isset($_POST['submit'])) {
     ...
     * 
     * Generally, we should always start validation with a persence check (i.e., did the user fill our this field?).
     * 
    ...
    */
    ...
    } elseif (!is_letter($name)) {
        $message_name = "<p class=\"text-warning\">Your name only contains letters and spaces.</p>";
    ...
    */
     ...
     * 
     * We can check to see if the provided values is a date by creating a DateTime object from it. 
     * 
     * We won't use strtotime() here because it silently fixes invalid dates instead of rejecting them.
    */

    if (!empty($dob)) {
        // Here, we'll attempted to create a DateTime object from the user input.
        ...
        } else { // if the $dob_obeject wasn't created or isn't the right format
     ...
     *  PASSWORDS
     * 
     * If we tell the user that we want certain things within a password, we should compare their input to the suitable regular expression.
     * 
    ...
    */
    ...
    if (is_blank($password)) {
        ...
        $message_password = "<p class=\"text-warning\">Your password must haveat least one lowercase letter.</p>"; 
    } elseif (!preg_match('/[0-9]/', $password)) {
        $message_password = "<p class=\"text-warning\">Your password must have at least one number.</p>"; 
    } elseif (!preg_match('/[\W_]/', $password)){
        $message_password = "<p class=\"text-warning\">Your password must include at least one of the following special character: !@#$%^&*.</p>"; 
    }
     ...
     * PASSWORD COMPARISON
     * 
     * This is relatively straightforward. Here, we want to see if the value that the user typed in the first field matches (or equal to) whatever they typed in the secon field.
    */
    ...
    */
    ...
    if (is_blank($region)) {
        ...
        $message_region = "<p class=\"text-warning\">That region name is a bit too long. Try something shorter (120 characters or fewer).</p>";
